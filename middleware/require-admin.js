 ...
 * - permissions (opcional): alcanza con tener uno de los permisos declarados
...
export default defineNuxtRouteMiddleware((to) => {
  ...
  if (!hasRoles && !hasUsernames && !hasPermissions) {
    if (import.meta.dev) {
  ...
  const hasAdminAccess = userCanAccessAdminRoute(userStore, {
    adminRoles: hasRoles ? adminRoles : [],
    allowedUsernames: hasUsernames ? allowedUsernames : [],
  })
  const hasPermission = hasPermissions && permissions.some((permission) => userHasPermission(userStore, permission))
