<template>
    ...
    <b-card no-body class="col-8 mt-1 section-card"  style="margin: 0px auto">
      ...
      <b-row class="align-items-center si-no-radios-row">
        <b-col lg="6" md="8" sm="8">
          <h5 class="mb-0">¿Sos representante o apoderado/a del solicitante? <i class="bi bi-question-circle-fill text-info field-help-icon" style="font-size: 1em" role="button" tabindex="0" @click.stop.prevent="openPopup('ApoderadoRepresentante')" @keydown.enter.stop.prevent="openPopup('ApoderadoRepresentante')"></i></h5>
        </b-col>
        <b-col cols="12" sm="auto" class="si-no-radios">
          <b-form-radio id="esApoderado-no" v-model="solicitante.esApoderado" name="radio-esApoderado" value="false">No</b-form-radio>
          ...
          <b-col lg="12" md="12">
            <b-form-group label="Nro de Legajo *" label-for="nroLegajo" >
              ...
              <div v-if="$v.nroLegajo.$error" class="validation-error">
                <i class="bi bi-exclamation-octagon text-danger"></i> El nro de legajo no puede estar vacío y debe contener solo números.
              </div>
            </b-form-group>
          </b-col>
        </b-row>
          ...
          <b-col lg="8" md="12">
            <b-form-group label="Calle *" label-for="direccion-inmueble-calle" >
              ...
              <div v-if="$v.inmueble.calle.$error" class="validation-error">
                <i class="bi bi-exclamation-octagon text-danger"></i> La calle no puede estar vacía.
              </div>
            </b-form-group>
          </b-col>
          <b-col lg="2" md="6">
            <b-form-group>
              <template #label>
                <span class="rubro-label">Número * <i class="bi bi-question-circle-fill text-info field-help-icon" style="font-size: 1em" role="button" tabindex="0" @click.stop.prevent="openPopup('NroInmueble')" @keydown.enter.stop.prevent="openPopup('NroInmueble')"></i></span>
              </template>
              ...
              <div v-if="$v.inmueble.nro.$error" class="validation-error">
                <i class="bi bi-exclamation-octagon text-danger"></i> El número no puede estar vacío, contener letras ni caracteres especiales.
              </div>
            </b-form-group>
          </b-col>
          <b-col lg="2" md="6">
      ...
      <fieldset  v-if="isHoteleria && ((solicitante.tipoSolicitud == 'Habilitación' || solicitante.tipoSolicitud == 'Cambio de Titular') || (solicitante.tipoSolicitud == 'Renovación' || solicitante.tipoSolicitud == 'Reempadronamiento'))" key="rubro-h">
        ...
        <p>Seleccioná los servicios que brinda tu establecimiento:</p>
          ...
          <div v-if="$v.inmueble.serviciosHoteleria.$error" class="validation-error">
            <i class="bi bi-exclamation-octagon text-danger"></i> Selecciona al menos una opción.
          </div>
        </b-form-group>
    ...
    <b-card no-body class="col-8 mt-1 section-card" style="margin: 0px auto">
      <fieldset >
        ...
        <b-form-group v-if="solicitante.tipoSolicitud=='Baja' || (solicitante.tipoSolicitud == 'Renovación' || solicitante.tipoSolicitud == 'Reempadronamiento')">
          <template #label>
            <span class="rubro-label">Libre Deuda de Ingresos Brutos <span v-if="solicitante.esTitular || (solicitante.tipoSolicitud == 'Renovación' || solicitante.tipoSolicitud == 'Reempadronamiento')">* </span><i class="bi bi-question-circle-fill text-info field-help-icon" style="font-size: 1em" role="button" tabindex="0" @click.stop.prevent="openPopup('ConstanciaLibreDeudaIB')" @keydown.enter.stop.prevent="openPopup('ConstanciaLibreDeudaIB')"></i></span>
          </template>
        ...
        <b-form-group v-if="solicitante.tipoSolicitud=='Baja' || (solicitante.tipoSolicitud == 'Renovación' || solicitante.tipoSolicitud == 'Reempadronamiento')">
          <template #label>
            <span class="rubro-label">Libre Deuda de Tasa de Inspección de Seguridad e Higiene. * <i class="bi bi-question-circle-fill text-info field-help-icon" style="font-size: 1em" role="button" tabindex="0" @click.stop.prevent="openPopup('ConstanciaLibreDeudaSegHig')" @keydown.enter.stop.prevent="openPopup('ConstanciaLibreDeudaSegHig')"></i></span>
          </template>
        ...
        <b-form-group>
          <template #label>
            <span class="rubro-label">Libre Deuda de Tasa por Servicios Urbanos <i>(o última factura de pago que indique que la Tasa municipal no registra deuda)</i>. * <i class="bi bi-question-circle-fill text-info field-help-icon" style="font-size: 1em" role="button" tabindex="0" @click.stop.prevent="openPopup('ConstanciaLibreDeuda')" @keydown.enter.stop.prevent="openPopup('ConstanciaLibreDeuda')"></i></span>
          </template>
        ...
        <b-row v-if="solicitante.tipoSolicitud == 'Cambio de Titular'" class="align-items-center si-no-radios-row">
          <b-col lg="6" md="8" sm="7">
            <h5 class="mb-0">¿Se hicieron modificaciones al plano?</h5>
          </b-col>
          <b-col cols="12" sm="auto" class="si-no-radios">
            <b-form-radio id="esModificacionesPlano-no" v-model="solicitante.esModificacionesPlano" name="radio-esModificacionesPlano" value="false">No</b-form-radio>
        ...
        <b-form-group v-if="(solicitante.tipoSolicitud == 'Renovación' || solicitante.tipoSolicitud == 'Reempadronamiento')">
          <template #label>
            <span class="rubro-label">Declaración Jurada - Metros Establecimiento Habilitado *<i class="bi bi-question-circle-fill text-info field-help-icon" style="font-size: 1em" role="button" tabindex="0" @click.stop.prevent="openPopup('decJurada')" @keydown.enter.stop.prevent="openPopup('decJurada')"></i></span>
          </template>
        ...
        <b-row class="align-items-center si-no-radios-row">
          <b-col lg="6" md="8" sm="7">
            <h5 class="mb-0">¿El iniciante es una Persona Jurídica?</h5>
          </b-col>
          <b-col cols="12" sm="auto" class="si-no-radios">
            <b-form-radio id="esPersonaJuridica-no" v-model="solicitante.esPersonaJuridica" name="radio-esPersonaJuridica" value="false">No</b-form-radio>
      ...
      <b-card v-if="solicitante.tipoSolicitud=='Renovación' || solicitante.tipoSolicitud=='Reempadronamiento'"  border-variant="warning" align="center" class="importante-card" >
        <b-card-text>
          <b-row >
            ...
            <b-col  md="10">
                <div class="li-row"><div class="li-icon"><i class="bi bi-caret-right-fill" style="font-size: 1em"></i></div><div class="li-content"><span>Una vez iniciado el trámite, <b>recibirás un correo electrónico del Departamento Comercio</b> (deptocomercio@gesell.gob.ar), indicandote los pasos para continuar.</span></div></div>
                <div class="li-row"><div class="li-icon"><i class="bi bi-caret-right-fill" style="font-size: 1em"></i></div><div class="li-content"><span>Tené en cuenta que la renovación ó el reempadronamiento serán efectivos una vez que el Departamento Comercio (deptocomercio@gesell.gob.ar) haya verificado la documentación presentada, se haya abonado el valor del trámite, se hayan presentado los originales y, posteriormente, hayas obtenido el certificado respectivo.</span></div></div>
                <div class="li-row"><div class="li-icon"><i class="bi bi-caret-right-fill" style="font-size: 1em"></i></div><div class="li-content"><span>Recordá que las <b>renovaciones y reempadronamientos</b> sólo se <b>realizan si no hiciste modificaciones</b> respecto a la <b>habilitación inicial</b>. En caso contrario deberás iniciar un  trámite diferente (por ejemplo, un cambio de domicilio).</span></div></div>
            </b-col>
          </b-row>
        </b-card-text>
      </b-card>
  ...
  <BModal v-model="showPopupApoderadoRepresentante" title="" :no-footer="true" @click-outside="showPopupApoderadoRepresentante = false" :header-bg-variant="'success'"  centered>
    <template #header>
      <div class="modal-info">
        <h5>
            <i class="bi bi-question-circle text-light"></i>
    ...
    <div class="modal-info popupApoderado">
      ...
      <p class="destacado"><i class="bi bi-caret-right-fill"></i>Esta figura permite facultar a una persona para la realización de trámites, actos y gestiones en representación del/la contribuyente o responsable solicitante.</p>
  ...
  <BModal v-model="showPopupConstanciaCUIT" title="Constancia de CUIT" :no-footer="true" @click-outside="showPopupConstanciaCUIT = false" :header-bg-variant="'success'"  centered>
    <template #header>
      <div class="modal-info">
        <h5>
            <i class="bi bi-question-circle text-light"></i>
    ...
    <div class="modal-info">
      <p><i class="bi bi-caret-right-fill"></i>Podes solicitar tu constancia de CUIT haciendo <a href="https://seti.afip.gob.ar/padron-puc-constancia-internet/ConsultaConstanciaAction.do" target="_blank" >click aquí</a>.</p>
    </div>
  </BModal>
  <BModal v-model="showPopupConstanciaIngresosBrutos" title="Constancia de Ingresos Brutos" :no-footer="true" @click-outside="showPopupConstanciaIngresosBrutos = false" :header-bg-variant="'success'"  centered>
    <template #header>
      <div class="modal-info">
        <h5>
            <i class="bi bi-question-circle text-light"></i>
  ...
  <BModal v-model="showPopupConstanciaLibreDeuda" title="Libre Deuda de Tasa por Servicios Urbanos" :no-footer="true" @click-outside="showPopupConstanciaLibreDeuda = false" :header-bg-variant="'success'"  centered>
    ...
    <div class="modal-info">
      <p><i class="bi bi-caret-right-fill"></i>Podés descargar tu última factura haciendo <a href="https://arvige.gob.ar/lpagos" target="_blank" >click aquí</a>.</p>
  ...
  <BModal v-model="showPopupCertificadoDomicilio" title="Certificado de domicilio Ingresos Brutos- Punto de Venta Villa Gesell" :no-footer="true" @click-outside="showPopupCertificadoDomicilio = false" :header-bg-variant="'success'"  centered>
    <template #header>
      <div class="modal-info">
        <h5>
            <i class="bi bi-question-circle text-light"></i>
    ...
    <div class="modal-info">
      <p><i class="bi bi-caret-right-fill"></i>Podés solicitar tu Certificado de domicilio haciendo <a href="https://www.arba.gov.ar/GuiaTramites/TramiteSeleccionado.asp?tramite=582&categ=34" target="_blank" >click aquí</a>.</p>
    </div>
  </BModal>
  <BModal v-model="showPopupConstanciaLibreDeudaIB" title="Libre deuda de Ingresos Brutos" :no-footer="true" @click-outside="showPopupConstanciaLibreDeudaIB = false" :header-bg-variant="'success'"  centered>
    <template #header>
      <div class="modal-info">
        <h5>
            <i class="bi bi-question-circle text-light"></i>
    ...
    <div class="modal-info">
      <p><i class="bi bi-caret-right-fill"></i>Podés solicitar tu Libre Deuda de Ingresos Brutos haciendo <a href="https://www.arba.gov.ar/GuiaTramites/TramiteSeleccionado.asp?tramite=266&categ=34#anclaDescripcion" target="_blank" >click aquí</a>.</p>
    </div>
  </BModal>
  ...
  <BModal v-model="showPopupConstanciaCambioTitular" title="Constancia de Conformidad de Cambio de Titularidad firmada por el anterior Titular" :no-footer="true" @click-outside="showPopupConstanciaCambioTitular = false" :header-bg-variant="'success'"  centered>
    <template #header>
      <div class="modal-info">
        <h5>
            <i class="bi bi-question-circle text-light"></i>
    ...
    <div class="modal-info">
      <p><i class="bi bi-caret-right-fill"></i>Podés descargar la planilla de la constancia de conformidad haciendo <a href="https://drive.google.com/file/d/1T0tLmahWr8YZ5q_XqV9_RPAkDGyXQ-wo/view" target="_blank" class="external-link">click acá</a>.</p>
    </div>
  </BModal>
  <BModal v-model="showPopupPlano" title="" :no-footer="true" @click-outside="showPopupPlano = false" :header-bg-variant="'success'"  centered>
    <template #header>
      <div class="modal-info">
        <h5>
            <i class="bi bi-question-circle text-light"></i>
  ...
  <BModal v-model="showPopupFormOk" title="" ok-only @click-outside="showPopupFormOk = false" :header-bg-variant="'success'" centered>
    <template #header>
    ...
    <div class="centeredContainer">
      ...
      <div class="centeredContainer" style="padding:0 1rem; padding-top: 1rem; border-top:1px solid #CCC; ">
        <p class="" style="text-align: justify; margin-bottom:0; font-size: 0.75rem "><i class="bi bi-caret-right-fill text-success"></i> Tené en cuenta que el Departamento Comercio puede solicitarte documentación adicional vía correo electrónico.</p>
