      ...
      <section v-if="resultado" class="periods-card">
        ...
        <div class="periods-heading">
          <div>
            ...
            <p>El PDF incluirá hasta dos boletas por página.</p>
          </div>
          <b-form-checkbox
        ...
        <div class="period-grid">
          <label
            v-for="periodo in resultado.periodos"
            :key="periodo.periodo"
            class="period-card"
            :class="{ selected: seleccionados.includes(periodo.periodo), disabled: descargando }"
          >
            <input v-model="seleccionados" type="checkbox" :value="periodo.periodo" :disabled="descargando">
        ...
        <div class="download-bar">
          <div>
            <strong>{{ seleccionados.length }} {{ seleccionados.length === 1 ? 'período seleccionado' : 'períodos seleccionados' }}</strong>
...
export default {
  ...
  computed: {
    ...
    todosSeleccionados() {
      return Boolean(this.resultado && this.seleccionados.length === this.resultado.periodos.length)
    },
  ...
  methods: {
    ...
    async buscar() {
      ...
      try {
        ...
        this.seleccionados = this.resultado.periodos.map(item => item.periodo)
      } catch (error) {
    ...
    seleccionarTodos(value) {
      this.seleccionados = value ? this.resultado.periodos.map(item => item.periodo) : []
    },
    limpiar() {
      this.resultado = null
      this.seleccionados = []
    ...
    async descargar() {
      ...
      try {
        ...
        this.mensajeError = 'No se pudo generar el PDF. Volvé a intentarlo.'
      } finally {
